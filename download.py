# Download the data from hugging face (make sure to enable permission in hugging face and create key)
#!/usr/bin/env python
import os
import requests
import tarfile
...
def main():
    token = "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" # My hugging face token
    for datatype, urls in DATA_URLS.items():
        
        # Store at root directory
        # destination = str(Path(__file__).parent / "data" / datatype)
        # os.makedirs(destination, exist_ok=True)
        
        # Store on a USB stick
        usb_path = "/Volumes/Training Data" 
        destination = os.path.join(usb_path, "data", datatype)
        os.makedirs(destination, exist_ok=True)
        
        for url in urls:
            # Grab whatever dataset set you need
            if datatype == "val":
